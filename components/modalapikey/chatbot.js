...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyButtons = document.getElementsByClassName("apply-button"); // No dot prefix
  const cancelButtons = document.getElementsByClassName("cancel-button"); // No dot prefix
  const projectsProfile = document.getElementById("projectsProfile");
  ...
  document.querySelector('[data-i18n="menu_projects"]').addEventListener("click", () => {
    projectsProfile.style.display = "block";
    userProfile.style.display = "none";
  });
  ...
  closeButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        e.target.closest(".modal-content").style.display = "none";
          console.log("Success");
      });
  });

  // Close modals when apply button is clicked
  Array.from(applyButtons).forEach((button) => { // Convert to array to use forEach
    button.addEventListener("click", (e) => {
      e.target.closest(".modal-content").style.display = "none";
    });
  });
  ...
  Array.from(cancelButtons).forEach((button) => { // Convert to array to use forEach
    button.addEventListener("click", (e) => {
      e.target.closest(".modal-content").style.display = "none";
     });
  });
